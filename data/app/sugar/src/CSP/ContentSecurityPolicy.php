...
class ContentSecurityPolicy
{
    ...
    public function withAddedDefaults(): ContentSecurityPolicy
    {
        ...
        $imgSrcDirective = Directive::create('img-src', 'data: http: https:');
        $new->setDirective($imgSrcDirective);
        $objectSrcDirective = Directive::create('object-src', "'self'");
        $new->setDirective($objectSrcDirective);
    ...
    {
        return [
            'default-src' => "'self' 'unsafe-inline' 'unsafe-eval' https://*.pendo.io *.sugarcrm.com *.salesfusion.com *.salesfusion360.com *.sugarapps.com *.sugarapps.eu",
            //Advanced form defaults
            'style-src' => "'self' 'unsafe-inline'",
            'script-src' => "'self' 'unsafe-inline' 'unsafe-eval' https://*.pendo.io",
            'base-uri' => "'self'",
            'form-action' => "'self'",
            'frame-src' => "'self'",
            'connect-src' => "'self' https://*.pendo.io",
            'font-src' => "'self'",
